...
/// Create outfit flow aligned with Android `CreateOutfitScreen`.
struct CreateOutfitSheet: View {
        ...
        VStack(spacing: 0) {
            ...
            if allItems.isEmpty {
                ContentUnavailableView(
                    "No wardrobe pieces",
                    systemImage: "tshirt",
                    description: Text("Add pieces in My Wardrobe first, then come back to build an outfit.")
                )
    ...
    private func save() {
        ...
        let outfit = Outfit(
            name: trimmedName,
            itemIdsJoined: Outfit.joinItemIds(orderedIds),
            wornCount: 0,
        )
